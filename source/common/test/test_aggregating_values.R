...
# The @series data frame contains 72 consecutive
# measurements of measurement_time, pm2_5 and temperature
# from 2018-01-01 00:00 UTC to 2018-01-03 23:00 UTC
# * $measurement_time - numeric values
# * $pm2_5 - numeric values equal to hour of the measurement_time (0 - 23)
# * $temperature - numberic values equal to 2 * hour of the measurement_time (0 - 46)

measurement_time <- seq(from = utcts('2018-01-01'), to = utcts('2018-01-03 23:00'), by = 'hour')
pm2_5 <- unlist(lapply(measurement_time, function (ts) {
  as.POSIXlt(ts)$hour
}))
temperature <- 2 * pm2_5
...
past_lag <- 6
future_lag <- 6
windows <- divide_into_windows(df = series, past_lag = past_lag, future_lag = future_lag, future_vars = future_vars)
aggregated_windows <- add_aggregated(windows, past_lag, excluded = c("measurement_time"))
...
test_that("future_measurement_time is equal to measurement_time + future_lag hours", {
  ...
  time_deltas <- unlist(lapply(1:nrow(windows), function(idx) {
    row <- windows[idx, ]
    difftime(row$future_measurement_time, row$measurement_time)
  }))
...
test_that("past values in window rows are equal to values in the original series", {
  ...
  calc_and_orig_past_values <- lapply(1:nrow(windows), function(idx) {
    ...
    compared_values_for_row <- lapply(1:nrow(past_col_to_delta), function(map_idx) {
      ...
      original_measurement_time <- utcts(row$measurement_time + (3600 * entry$delta))
      data.frame(
        original_name = entry$original_name,
        original_measurement_time = original_measurement_time,
        measurement_time = row$measurement_time,
        calculated_value = as.numeric(row[, entry$past_name]),
        original_value = as.numeric(series[series$measurement_time == original_measurement_time, entry$original_name])
      )
    })
...
test_that("aggregated min PM2.5 value is equal to the min value in the group of values", {
  windows <- add_aggregated(windows, past_lag, excluded = c("measurement_time"))
  expect_true(
    all(
...
test_that("aggregation ignores NA values ", {
  ...
  # We remove the first and last PM2.5 values in the first time window
  ...
  expect_true(aggregated_windows[1, "min_7_pm2_5"] == df[2, "pm2_5"] &&
    aggregated_windows[1, "mean_7_pm2_5"] == mean(seq(1, future_lag - 1)) &&
