...
source("constants.R")
source("preprocess.R")
packages <- c("ggplot2", "reshape", "car", "scales", "ggthemes", "moments", "viridis")
import(packages)
...
save_multiple_vars_plot <- function(df, x_var, y_var, id_var, plot_path, x_lab = "", y_lab = "", breaks = "4 months", legend_position = "none") {
...
save_heatmap <- function(df, var_x, var_y, plot_path, title = "", show_trend = FALSE) {
  plot <- ggplot(data = df, mapping = aes_string(x = var_x, y = var_y)) +
    stat_binhex() +
    scale_fill_viridis() +
    xlab(get_or_generate_label(var_x)) +
    ylab(get_or_generate_label(var_y)) +
    ggtitle(title)
  plot <- if (show_trend) {
    plot + geom_smooth(method = "lm", na.rm = TRUE, color = COLOR_CONTRAST)
  } else {
    plot
  }
  save_plot_file(plot, plot_path)
...
save_data_split <- function(res_var, training_set, validation_set, test_set, plot_path, breaks = "4 months", legend_position = "right") {
  cols_to_extract <- c("measurement_time", res_var) 
  
  training_vals <- training_set[, cols_to_extract]
  training_vals$type <- "training"
  
  validation_vals <- validation_set[, cols_to_extract]
  validation_vals$type <- "validation"
  
  test_vals <- test_set[, cols_to_extract]
  test_vals$type <- "test"
  
  merged <- rbind(training_vals, validation_vals, test_vals)
  xlab <- "Date"
  save_multiple_vars_plot(
    df = merged,
