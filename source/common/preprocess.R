...
reverse_normalize_with <- function(data, mins, maxs) {
  ...
  sapply(colnames(data), function(col) {
    data[, col] * (maxs[col] - mins[col]) + mins[col]
  })
}
...
add_aggregated <- function(windows, past_lag, vars_to_aggregate = c(), excluded = c(), aggregate_only_complete = FALSE) {
  if (past_lag <= 0) {
    windows
  }
  ...
  all_stats <- lapply(vars, function(v) {
    ...
    stats <- apply(sliced, 1, function(row) {
      row_stats <- unlist(lapply(aggr_funs, function(f) {
        ...
        # If aggregate_only_complete is set to TRUE the final aggregated
        # value (assigned to all_stats) is set to NA if any value
        # in a row is incomplete
        suppressWarnings(f(row, na.rm = !aggregate_only_complete))
      }))
      row_stats
    })
...
skip_past <- function(windows, excluded = c()) {
  ...
  excluded <- unlist(
    lapply(excluded, function(e) {
      past_vars[grepl(e, past_vars)]
    })
  )
